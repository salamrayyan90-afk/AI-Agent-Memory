...
def fetch_url_text(url: str) -> tuple[bool, str]:
    """
    Web scraper: fetch HTML from URL and return plain text for AI analysis.
    Uses requests + BeautifulSoup. Works on Streamlit Cloud (respects HTTP_PROXY/HTTPS_PROXY if set).
    Returns (success, text_or_error_message). Content is passed to Groq for Q&A via GROQ_API_KEY.
    """
    ...
    try:
        ...
        # Proxy: requests uses HTTP_PROXY / HTTPS_PROXY from env on Streamlit Cloud when set
        session = requests.Session()
        session.headers.update({"User-Agent": "Mozilla/5.0 (compatible; CORE-AI/1.0; Streamlit)"})
...
""" + knowledge_base[:8000] + "\n\n---\n\n"
    ...
    web_fetched_block = ""
    if web_fetched_content and web_fetched_content.strip():
        web_fetched_block = """
**Web Intelligence (user-fetched page)** — The user has fetched this URL for analysis: """ + (web_fetched_url or "—") + """

Use the text below to answer questions about this specific site. Summarize, compare, or extract information as asked.

""" + web_fetched_content[:30000] + "\n\n---\n\n"
    manifesto_block = ""
    ...
    return (
        (expertise + "\n\n---\n\n") if expertise else ""
    ) + manifesto_block + rag_block + vector_rag_block + research_block + web_fetched_block + kb_block + """You are the Core AI Reasoning Agent. Follow this cognitive style in every response.
...
def load_sidebar() -> None:
    ...
    with st.sidebar:
        ...
        st.divider()
        st.markdown('<div style="font-size:0.75rem;text-transform:uppercase;color:var(--text-secondary);margin:24px 0 8px 0;font-weight:600;">Browse Website</div>', unsafe_allow_html=True)
        browse_url = st.text_input(
            "Browse Website",
            placeholder="Enter URL — CORE AI will read it and answer questions",
            key="website_viewer_url",
            label_visibility="collapsed",
        )
        col_v, col_f = st.columns(2)
        with col_v:
            if st.button("View website", key="view_website_btn", use_container_width=True) and browse_url:
                st.session_state.viewer_url = browse_url.strip()
                st.rerun()
        with col_f:
            if st.button("Fetch & Analyze", key="fetch_analyze_btn", use_container_width=True) and browse_url:
                url_to_fetch = browse_url.strip()
                with st.spinner("Fetching..."):
                    ok, result = fetch_url_text(url_to_fetch)
                if ok:
                    st.session_state.fetched_site_content = result
                    st.session_state.fetched_site_url = url_to_fetch if url_to_fetch.startswith(("http://", "https://")) else "https://" + url_to_fetch
                    st.success("Loaded. Ask questions about this site in chat.")
                else:
                    st.error(f"Fetch failed: {result}")
                st.rerun()
        if st.session_state.get("fetched_site_url"):
            st.caption(f"Analyzing: {st.session_state.fetched_site_url[:48]}...")
...
load_background()
load_sidebar()
# Website Viewer: if a URL was entered in sidebar, show it in an iframe in the main area
viewer_url = st.session_state.get("viewer_url", "").strip()
if viewer_url:
    ...
    try:
        ...
        components_iframe(viewer_url, height=800, scrolling=True)
    except Exception as e:
        st.error(f"Could not load URL: {e}")
    st.caption("**Note:** Some sites (e.g. Google, Facebook) block embedding in iframes for security. If the page is blank, the site may not allow it.")
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                system_content = build_reasoning_agent_system(
                    ...
                    web_fetched_content=st.session_state.get("fetched_site_content", ""),
                    web_fetched_url=st.session_state.get("fetched_site_url", ""),
                )
